    ...
    <g transform={`translate(${x},${y})`}>
      {wrappedLines.map((line, i) => (
        <text
          ...
          x={-10}
          y={0}
          dy={i * 14 - ((wrappedLines.length - 1) * 7)}
          textAnchor="end"
          fill="#f1f5f9"
          fontSize={12}
          fontWeight={500}
...
const OUTCOME_DATA = [
    ...
    { name: 'Individual Contributor (0)', value: 64.56 },
]; 
...
const DataVizPanel: React.FC<DataVizPanelProps> = ({ activeScenario }) => {
  ...
  const renderScenario0 = () => {
    ...
    return (
        <div className="h-full flex flex-col text-white">
          <div className="mb-6">
             <h3 className="text-white text-base font-semibold md:text-2xl md:font-bold mb-2">Highest level reached in career</h3>
          </div>
          <div className="flex-grow min-h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart 
                data={OUTCOME_DATA} 
                layout="vertical" 
                margin={{ top: 10, right: 60, left: isMobile ? -20 : 0, bottom: 20 }}
                barCategoryGap="33%"
              >
                ...
                <YAxis 
                    type="category" 
                    dataKey="name" 
                    tick={isMobile ? <CustomYAxisTick /> : {fill: '#f1f5f9', fontSize: 14, fontWeight: 500}}
    ...
    const renderFacetChart = () => {
        const highlightRowIndex = activeScenario === 1 ? 0 : 3;
        const isMobile = typeof window !== 'undefined' && window.innerWidth < 768;
    ...
    return (
        <div className="h-full flex flex-col text-slate-200 overflow-hidden relative">
             <div className="mb-4">
                 <h3 className="text-white text-base font-semibold md:text-2xl md:font-bold mb-2">Merit vs luck: career outcomes</h3>
             </div>

                 <div className="grid gap-4 mb-4 border-b border-slate-700 pb-2" style={{ gridTemplateColumns: colTemplate }}>
                <div className="font-bold text-xs text-slate-500 uppercase tracking-wider self-end">Merit Level</div>
                     <div className="text-center font-bold text-xs md:text-sm text-slate-300">100% Meritocracy</div>
                     <div className="text-center font-bold text-xs md:text-sm text-slate-300">50% Luck</div>
                     <div className="text-center font-bold text-xs md:text-sm text-slate-300">100% Luck</div>
             </div>
                         ...
                         <div className="flex-grow flex flex-col justify-between">
                             {(() => {
                                    let rows = FACET_DATA;
                                    if (isMobile) {
                                        // Refine threshold: previous value (~605px) hid Top 25th Percentile on medium-height screens (e.g. 70vh phones).
                                        ...
                                        // Use a slightly lower required height so 3 rows appear when panel >= 560px.
                                        const requiredForThree = 500; 
                                    ...
                                    return rows.map((row, idx) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className={`text-xs font-semibold leading-tight pr-2 transition-colors duration-500 ${isRowHighlighted ? 'text-blue-400 font-bold' : 'text-slate-400'}`}>{row.label}</div>
                                                {row.cells.map((cell, cIdx) => (
                                                    <div key={cIdx} className="h-[120px] border-l border-slate-700 pl-2 relative flex flex-col justify-center gap-1">
                                                        {cell.dist.map(d => {
                                                            ...
                                                            return (
                                                                <div
                                                                    key={d.level}
                                                                    className="flex items-center gap-1 h-[12px] text-[10px] group cursor-pointer transition-transform duration-200"
                                                                    onMouseEnter={() => setHoveredBarId(barId)}
                                                                    onMouseLeave={handleBarLeave}
                                                                >
                                                                    {!(isMobile && cIdx > 0) && (
                                                                        <div className="w-5 text-right text-slate-500 font-mono text-[9px]">{LEVEL_ABBREVS[d.level]}</div>
                                                                    )}
                                                                    <div className="flex-grow h-full bg-slate-800 relative rounded-sm overflow-hidden">
                                                                        <div
                                                                    ...
                                                                    <div className={`w-8 font-medium transition-all duration-300 origin-left ${textColor}`}>
  ...
  const renderMedianProgression = () => {
        ...
        <div className="h-full flex flex-col text-white">
            <div className="mb-2">
                <h3 className="text-white text-base font-semibold md:text-2xl md:font-bold mb-2">Median performer, 50% luck</h3>
                
                <div className="flex items-center gap-6 my-4 text-xs">
                    <div className="flex items-center gap-2">
                        <span className="w-3 h-3 bg-slate-500 rounded-sm opacity-80"></span>
                        <span className="text-slate-300">Individual Contributor</span>
                    </div>
                    <div className="flex items-center gap-2">
                ...
                <p className="text-slate-400 text-sm">% employees at level</p>
            </div>
            <div className="flex-grow min-h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                    ...
                    >
                        ...
                        <XAxis 
                            ...
                            label={{ value: 'Career phase out of 5', position: 'insideBottom', offset: -10, fill: '#94a3b8', fontSize: 13 }}
                        />
                        <YAxis 
                            tick={isMobile ? undefined : {fill: '#94a3b8', fontSize: 13}} 
                            hide={isMobile}
                            tickLine={false}
                            axisLine={false}
                            tickFormatter={(val) => `${val}%`}
  ...
  return (
    <div 
        ref={panelRef}
                className="w-full h-full p-6 md:p-8 flex flex-col bg-slate-900 rounded-3xl shadow-[0_30px_80px_rgba(15,23,42,0.9)] border border-slate-800/80 transition-colors duration-500 relative"
    >
